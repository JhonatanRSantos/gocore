...
func TestLoadString(t *testing.T) {
	...
	if Load("TEST_APP_STRING", "DEFAULT_STRING_VALUE") != "STRING_VALUE" {
		assert.FailNow(t, "failed to load env var (string)")
	}
}
...
func TestLoadInt(t *testing.T) {
	for i := 0; i < 10; i++ {
		...
		if value != int64(i) {
			assert.FailNow(t, "failed to load env var (int64)")
		}
	}
}
...
func TestLoadFloat(t *testing.T) {
	for i := 0; i < 10; i++ {
		...
		if value != float64(i)+0.75 {
			assert.FailNow(t, "failed to load env var (float64)")
		}
	}
}
...
func TestLoadBool(t *testing.T) {
	...
	if value != true {
		assert.FailNow(t, "failed to load env var (bool)")
	}
}
